...
require _TPL_TOP_;
?>
    <h1 class="mb-3"><small>IA Anael</small></h1>
    <?php
...
<?php endif; ?>
    ...
    <script type="module">
        const model = await tf.loadLayersModel('<?=_URL_ADMIN_ ?>ia_anael/model.json');
        ...
        for (const unTr of tabTr) {
            ...
            // Redimensionner notre image à la taille utilisée pour entraîner le modèle
            ...
            if (predictions[0] > 0.5) {
                result += 'OK -> ' + Math.round(predictions[0] * 100) + '%'
            } else {
                result += 'A BLOQUER -> ' + Math.round(predictions[1] * 100) + '%'
            }
            // Injection de la valeur dans la page
            unTr.children[3].innerHTML = result;
        }
    </script>
    <script>
        /**
