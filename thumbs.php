...
function make_thumbs($path, $p_image, $t_name, $width, $height, $t_height, $t_width) {
    ...
    if (
            !(($extension == 'jpg') && (!$image = @imagecreatefromjpeg($p_image))) && !(($extension == 'gif') && (!$image = @imagecreatefromgif($p_image))) && !(($extension == 'png') && (!$image = @imagecreatefrompng($p_image)))
    ) { //2 req FALSE sur $extension, troisieme > image..., imagecreatefrom retourne FALSE en cas d'erreur // charge en m�moire une image du fichier
        //if(($miniature = @imagecreatetruecolor($t_width, $t_height)) != FALSE)
        if (($miniature = imagecreatetruecolor($t_width, $t_height)) != FALSE) { // cr�ation image vierge
            //if(@imagecopyresampled($miniature, $image, 0,0, 0,0, $t_width,$t_height, $width,$height))
            if (imagecopyresampled($miniature, $image, 0, 0, 0, 0, $t_width, $t_height, $width, $height)) { // On cr�e la miniature
                if ($extension == 'jpg') { //miniature jpg
                    if (@imagejpeg($miniature, $path . $t_name, 100)) {// On enregistre la miniature
                        $retour = 1;
                    ...
                    if (@imagegif($miniature, $path . $t_name)) { //pas de gestion de qualit� en gif// On enregistre la miniature
                        $retour = 1;
                    } else { //Erreur � l'enregistrement
                        retour_erreur('<p>Erreur #0 : Impossible d\'enregistrer le fichier ' . $t_name . '</p>', __FILE__, 'warning');
                    }
                } elseif ($extension == 'png') {
                    if (@imagepng($miniature, $path . $t_name, 9)) {// On enregistre la miniature
                        $retour = 1;
                    } else { //Erreur � l'enregistrement
                        retour_erreur('<p>Erreur #0 : Impossible d\'enregistrer le fichier ' . $t_name . '</p>', __FILE__, 'warning');
                    }
